...
export default function MultiplayerGame({ multiplayer, onBack }: MultiplayerGameProps) {
  ...
  const settlementKey = isEnded && multiplayer.gameEndReason
    ? `${multiplayer.gameEndReason}:${multiplayer.winner ?? 'draw'}:${multiplayer.moveNumber}:${multiplayer.gameStartTime ?? 0}`
    : null
  ...
  useEffect(() => {
    ...
    if (settlementKeyRef.current === settlementKey) return
    settlementKeyRef.current = settlementKey
  ...
  }, [iWin, multiplayer.gameStartTime, multiplayer.winner, playDefeat, playWin, settlementKey])
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setTurnRemaining((prev) => (prev === next ? prev : next))
  ...
  useEffect(() => {
    ...
    if (!emojiData && !emojiDataLoading) {
      setEmojiDataLoading(true)
      import('@emoji-mart/data')
        .then((mod) => setEmojiData(mod.default as Record<string, unknown>))
        .finally(() => setEmojiDataLoading(false))
    }
  ...
  }, [emojiData, emojiDataLoading, showEmojiPanel])
  ...
  const handleAcceptRestartInvite = () => {
    playClick()
    setRestartDecisionPending(true)
    multiplayer.acceptRestart()
  }

  const handleDismissRestartInvite = () => {
    playClick()
    setRestartDecisionPending(true)
    ...
    <main className={`${fusion.page} pt-safe-top pb-safe-bottom`}>
      <div style={{ maxWidth: 430, margin: '0 auto', padding: `0 ${horizontalPadding}px 20px` }}>
        <div className={fusion.content}>
            ...
            {showEmojiPanel && (
              ...
              >
                ...
                {emojiData ? (
                  ...
                  <div style={{ padding: 12, color: '#4b5563', fontSize: 12 }}>
                    {emojiDataLoading ? '正在加载表情...' : '表情数据未就绪'}
                  </div>
                )}
              </div>
            )}
          ...
          {!hasRestartPrompt && multiplayer.restartNotice && <div className="status-banner status-banner-info px-3 py-2 text-sm">{multiplayer.restartNotice}</div>}
          ...
          {multiplayer.pendingRestartRequestFrom && (
            <>
              <div className="status-banner status-banner-info px-3 py-3 text-sm">
                <div style={{ fontWeight: 900, marginBottom: 4 }}>对手请求再来一局</div>
                <div>{multiplayer.pendingRestartRequestFrom} 邀请你立即开始新对局。</div>
              </div>
              <div className={fusion.buttonGrid}>
                <button className={fusion.buttonPrimary} type="button" disabled={restartDecisionPending} onClick={handleAcceptRestartInvite}>
                  {restartDecisionPending ? '处理中...' : '接受再来一局'}
                </button>
                <button className={`${fusion.buttonSecondary} ${fusion.buttonLight}`} type="button" disabled={restartDecisionPending} onClick={handleDismissRestartInvite}>
                  {restartDecisionPending ? '处理中...' : '稍后'}
                </button>
              </div>
            </>
          )}
      ...
      <VictoryOverlay
        ...
        resultInfoOverride={
          multiplayer.gameEndReason === 'resign' && multiplayer.winner && !multiplayer.winningLine
            ? {
        ...
        extraContent={
          hasRestartPrompt ? (
            <div className="status-banner status-banner-info px-4 py-3 text-sm" style={{ marginTop: 10 }}>
              <div style={{ fontWeight: 900, marginBottom: 4 }}>对手请求再来一局</div>
              <div>{multiplayer.pendingRestartRequestFrom} 邀请你立即开始新对局。</div>
            </div>
            ...
            <div className="status-banner status-banner-info px-4 py-3 text-sm" style={{ marginTop: 10, textAlign: 'center' }}>
              已邀请对方再来一局，等待确认。
            </div>
          ) : null
        }
